...
$(document).ready(function () {
  ...
  $('.imgLiquidFill img').on('load', function () {
    ...
    $(this).parent('.imgLiquidFill').addClass('ed'); //console.log(i++);
  }).each(function () {
    if (this.complete) $(this).load();
  });
  var loadn = 0;
  var allimgs = $('img').length; // $('img').on('load', function() {
  //     $(this).parent('.imgLiquidFill').imgLiquid();
  //     loadn++;
  ...
  $('.top_menu .subheader').on('mouseenter', function () {
    if (windowWidth > 480) {
      if (!$(this).find('a').hasClass('on')) {
        $('.top_menu>ul>li a.on').next('.submenu').stop().slideUp(200, 'easeInOutSine', function () {
  ...
  $('.serMenu .subheader').on('click', function () {
    ...
    $(this).addClass('on');
    $(this).children('ul').slideDown(400, 'easeInOutSine'); // if($(this).hasClass('open')){
  ...
  $('.qabox ul>li').on('click', function (event) {
    ...
    if (!$(this).children('.qa_list').hasClass('show')) {
      ...
      $('.qabox ul>li .qa_content').stop().slideUp(400); //打開
